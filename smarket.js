/* ============================================
   SMARKET FULL-STACK GALLERY INTERACTIVITY
   ============================================ */
...
const imageWrapper = document.getElementById('image-wrapper'); // Get the image wrapper
...
function openLightbox(initialSrc, title, allImagesString = '') {
    ...
    // Handle Gallery vs Single Image
    if (allImagesString) {
        const srcs = allImagesString.split(',').map(s => s.trim());
        lightboxImages = srcs.map(src => ({
            src: src,
            title: title // Reusing main title or you can customize
        }));
    ...
    lightboxImg.style.transition = 'none'; // Disable transition for instant reset
...
function zoomImage(factor) {
    ...
    // Progressive zoom: addition instead of multiplication for smoother control
...
lightboxImg.addEventListener('mousedown', (e) => {
    if (currentZoom > 1) {
        ...
        lightboxImg.style.transition = 'none'; // Instant response while dragging
    }
});
...
function initBackToTop() {
    ...
    const customCursor = document.getElementById('custom-cursor'); // Get a reference to the custom cursor
    ...
    backToTopBtn.addEventListener('click', () => {
        ...
        if (customCursor) {
            customCursor.classList.add('clicking'); // Add the clicking class
            if (typeof playSound === 'function') playSound('pop'); // Play the click sound
            setTimeout(() => {
                customCursor.classList.remove('clicking'); // Remove after a short delay
            }, 200); // Match or slightly exceed the ghost's click animation duration
        }
    });
}
